      ...
      class << self
        ...
        def parse_type_and_options(type)
          ...
          when /(string|text|binary|integer)\{(.+)\}/
            type = $1
            options = parse_provided_options($2)
            return type, options
          when /decimal\{(\d+)[,.-](\d+)\}/
        ...
        def parse_provided_options(opts)
          provided_options = opts.split(/[,.-]/)
          options = { }
          provided_options.each do |option|
            k = option.split("=").first
            v = option.split("=").last
            options[k.to_sym] = parse_option_value(v)
          end
          options
        end
